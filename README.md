...
- `bensz-notes-vibe-config`：通过 `bensz-notes` 的 DevTools Agent API Token 管理笔记、目录、标签、本地优先的 Markdown 工作区上传/镜像同步、设置、成员、token 和审计入口
...
### `skills/bensz-notes-vibe-config`

- 来源：`/Volumes/2T01/Github/bensz-notes`
- 目标系统：`bensz-notes`
- 能力范围：笔记、目录、标签、本地优先的 Markdown 工作区上传/镜像同步（含路径变更）、设置、成员、Agent token、审计与平台治理只读入口
- 安全边界：仅访问配置的 DevTools/API Token 后端接口，默认 `{BENSZ_NOTES_URL}/api/backend/*`
